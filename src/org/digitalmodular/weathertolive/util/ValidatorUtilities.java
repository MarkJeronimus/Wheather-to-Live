...
public final class ValidatorUtilities {
	...
	public static <C extends Collection<T>, T> @Nullable C requireNullOrNotEmpty(@Nullable C actual, String varName) {
	...
	public static <C extends Collection<T>, T> C requireSizeAtLeast(int min, C actual, String varName) {
		if (actual.size() < min) {
			throw new IllegalArgumentException(
	...
	public static <M extends Map<K, V>, K, V> M requireSizeAtLeast(int min, M actual, String varName) {
		if (actual.size() < min) {
			throw new IllegalArgumentException(
					'\'' + varName + "' must contain at least " + min + " elements: " + actual.size());
		}
